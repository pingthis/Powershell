...
$eMailAddress = 'rdewberry@hbl2.com'
...
if (Get-IsValidEmailAddress $eMailAddress){
    ...
    Connect-EX $Jobuser
    $Messages = Get-MessageTrace -RecipientAddress $eMailAddress -StartDate ([datetime]::Now).AddDays(-15) -EndDate ([datetime]::Now) | Out-String
}else {
    write-host "Oh NO! INVALID EMAIL ADDRESS"
}
Disconnect-ExchangeOnline -Confirm:$False
Send-MailMessage -To "Markthompson@cameronashleybp.com"  -From 'CABPDOCDIST@CAMERONASHLEYBP.COM' -SmtpServer $SMTPServer -Subject 'Bad Mail' -Body $Messages
